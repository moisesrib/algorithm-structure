    ...
    <svg width="100" height="60" viewBox="0 0 100 60" xmlns="http://www.w3.org/2000/svg">
      ...
      <defs>
        <filter id="glow" x="-20%" y="-20%" width="140%" height="140%">
          <feDropShadow dx="0" dy="0" stdDeviation="4" flood-color="${swap ? '#FFD700' : '#00E1FF'}" flood-opacity="1"/>
        </filter>
      </defs>
    </svg>
  `
...
const generateRandomCars = () => {
  ...
  swaps.value = 0
  comparisons.value = 0
  cars.value.forEach((car: Car, idx: number) => { car.x = idx * carSpacing })
}
...
const animateSwapX = async (carA: Car, carB: Car, idxA: number, idxB: number) => {
...
const animateToFinalOrder = async (finalOrder: number[]) => {
  ...
  for (let i = 0; i < finalOrder.length; i++) {
    ...
    while (currentIdx > i) {
      await animateSwapX(cars.value[currentIdx], cars.value[currentIdx - 1], currentIdx, currentIdx - 1);
      [cars.value[currentIdx], cars.value[currentIdx - 1]] = [cars.value[currentIdx - 1], cars.value[currentIdx]];
    ...
    <Card class="mb-6">
      ...
      <CardContent>
        <div class="flex gap-4">
          <Button @click="ordenarComBubbleSort" variant="default">Ordenar (bubbleSort)</Button>
        </div>
      </CardContent>
    </Card>
    ...
    <div class="flex gap-8">
      <Card class="flex-1">
        <CardContent class="p-0">
          <div class="relative" :style="{height: TRACK_HEIGHT + 'px', width: TRACK_WIDTH + 'px'}">
            <div v-for="i in NUM_CARS" :key="'lane-bg-' + i" :style="{height: LANE_HEIGHT + 'px', top: (i-1)*LANE_HEIGHT + 'px'}" class="absolute left-0 w-full" :class="i % 2 === 0 ? 'bg-gray-800' : 'bg-gray-700'"/>
            <div v-for="i in NUM_CARS" :key="'lane-line-' + i" :style="{top: i*LANE_HEIGHT + 'px'}" class="absolute left-0 w-full h-[2px] bg-white/20"/>
            <div 
              ...
              :style="{
                left: `${car.x}px`,
                top: `${(NUM_CARS - 1 - idx) * LANE_HEIGHT + (LANE_HEIGHT - 60) / 2}px`,
                width: '100px',
                height: '60px',
                zIndex: 10,
                ...(comparing.includes(idx) ? { 'ring-4 ring-yellow-400' : '' } : {})
              }"
            >
              <div v-html="generateCarSVG(car.color, comparing.includes(idx), comparing.includes(idx) && cars[comparing[0]] && cars[comparing[1]] && cars[comparing[0]].speed > cars[comparing[1]].speed)" class="w-full h-full"></div>
              <div class="absolute -left-32 top-1/2 -translate-y-1/2 text-white font-bold bg-black/50 px-2 py-1 rounded text-xs whitespace-nowrap">
                {{ car.speed }} km/h
              </div>
            </div>
          </div>
    ...
    <Card class="mt-6">
      ...
      <CardContent>
        <p class="text-gray-600">
          O Bubble Sort compara dois carros adjacentes por vez. Se o da frente for mais lento, eles trocam de posição. O processo se repete até todos estarem ordenados por velocidade!
        </p>
      </CardContent>
    </Card>
  </div>
